...
const AddProductScreen = ({ route }) => {
    ...
    const { uploadImages, addDocument, getReference, getDocumentById, updateDocument } = useFirebase()
    ...
    const addProduct = async () => {
        ...
        const wRes = await getDocumentById("wallet", user?.uid)

        if (wRes.status === 400) {

            Alert.alert(
                "Error",
                "You need to add your wallet details first",
                [
                    {
                        text: "OK",
        ...
        if (expert && wallet.balance < 10000) {
            Alert.alert(
                "Error",
                "You need to have atleast 10000 PKR in your wallet to post an expert ad",
                [
                    {
                        text: "OK",
                        onPress: () => navigation.navigate('Wallet'),
        ...
        if (!expert && wallet.balance < 200) {
            Alert.alert(
                "Error",
                "You need to have atleast 200 PKR in your wallet to post an ad",
                [
                    {
                        text: "OK",
                        onPress: () => navigation.navigate('Wallet'),
        ...
        if (expert) {
            await addDocument("payment", {
                ...
                user: user.uid,
                status: "success",
                description: "Expert Ad Payment"
            })

            await updateDocument("wallet", user?.uid, {
        ...
        const res = await addDocument("products", product)
