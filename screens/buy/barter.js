...
const ProductDetailScreen = ({ route }) => {
    ...
    const { title, price, images, _id } = route.params.product;
    ...
    const handleSubmit = async () => {
        ...
        await axios
            .post('/bid', {
                product: _id,
                offeredProducts: selected,
                cash: cash,
                buyer: user.id,
                seller: route.params.product.user._id
            })
            .then((response) => {
                ...
                // navigation.goBack()
                navigation.navigate('BidingRequest', {
                    screen: 'BidBuyer',
                })
            })
            .catch((error) => {
    ...
    const getProducts = async () => {
        setProducts([])
        await axios
            .get('product/my-products/' + user.id + "?status=active,not-for-sale")
            .then((response) => {
...

            {
                addModal == "furthercategory" && (
                    <FurCategoryModal
                        ...
                        selectFurcategory={(furthercategory) => {
                            setFurthercategory({
                                id: furthercategory._id,
                                name: furthercategory.name,
                                inputs: furthercategory.inputs
                            })
                            setAddModal('product')
                        }}
                        category={category.id}
...

            {
                addModal === "product" && (
                    <ProductModal
                        ...
                        inputs={[...(category.inputs || []), ...(subcategory.inputs || []), ...(furthercategory.inputs || [])]}
